#!/usr/bin/python
# Written by John Patterson, yesthatjohn@hotmail.com
# multi-threaded animated mandala (or random image) maker.
# Run it like this from the command line:
# ./november_025.py 90 0 1 1

#  args: 
#  1) number of frames sys.argv[0]
#  2) flag to display sys.argv[1]
#  3) flag to render sys.argv[2]
#  4) sys.argv[3] make animated gif 0/1
...
def pixelCalc(q,r):
    ...
    B=60.
    G=200.
    #if q +sin(radians(frame*16)) > 0:
    #    R=100.
    #    G=60.
    #    B=255.

    #if (dist+dist*.8*sin(.01*radians(pow(.01*dist, .25)+frame*4))) <20000:
    ...
    G = (1.0+sin(radians(frame*4)))*(q/628.) * max(0, 120*(1.0-.0235*pow(4*(-.25+(q +dist*.5*sin(.1*radians(pow(dist,.5)+frame*4)))%3000/15000.0), 1)))  
    R = min(R,255)
    G = min(G,255)
    B = min(B,255)
    B_temp = B
    ...
    #if threshold > .5:
    #    R = R + (threshold - .5)*(255 - 50*pow(dist/(92500.), 1.2))
    #    G = G + (threshold - .5)*(255 - 50*(pow(dist,.85)/(92500.)))
    #    B = B + (threshold - .5)*(100 + 55*(dist/(92500.)))
    if R < 30 and R > 10:
        R = (30-R)*(R-10)
        G = (30-R)*(R-10)*(q/628.)
        B = 255-.05*(30-R)*(R-10)
...
if __name__=='__main__':
    ...
    for frame in range(frames):
        ...
        #print RGB[0]
        image_array_munger(int(sys.argv[2]), int(sys.argv[3]), int(sys.argv[4]))  # assemble image data array, argv[2] = show?, argv[3] = save?
